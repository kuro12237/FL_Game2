...
void EnemyWalkManager::Initialize()
{
   ...
   for (uint32_t Count_ = 0; Count_ < uint32_t(gameObjectManager_->GetObj3dDatas().size());
      ...
      if (gameObjectManager_->GetObj3dDatas().find(enemyName) !=
        ...
        shared_ptr<EnemyWalk> enemyWalk = nullptr;
   enemyWalk = make_shared<EnemyWalk>();
   enemyWalk->INameable::SetName(name);
  
   	// オブジェクトの作成
   shared_ptr<Game3dObjectData> data = make_shared<Game3dObjectData>();
   data->SetObjectType("MESH");
   uint32_t modelHandle_ = Engine::Manager::ModelManager::LoadObjectFile("PlayerNormalBullet");
   const Engine::Transform::TransformEular transform = {{1.0f, 1.0f, 1.0f}, {}, {0,10,0}};
   data->Initialize(transform, {}, modelHandle_);

   data->SetObjName(name);
   GameObjectManager::GetInstance()->PushObj3dData(data, name);
